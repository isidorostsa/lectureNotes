...
function finalValue = ex_2_1(func, gradfunc, max_iterations, p0, gamma_selection_method)
    plist = [];
    glist = [];
    p = p0;
    plist = [plist p];
    ...
    grad = [1; 1];
    while norm(grad) > 1e-6 && max_iterations > 0
        ...
        elseif(gamma_selection_method == 2)
            gamma = fminbnd(@(gamma) func(p - gamma*grad), 0, 3);
