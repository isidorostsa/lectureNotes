...
2. Now filter for size:
	...
	this number is the size of `5MiB` in characters $-1$ . The `+` is used to only keep those with greater size, and the `c` at the end means size in `char`. This will print the files in different lines. This is hard to 
3. 
